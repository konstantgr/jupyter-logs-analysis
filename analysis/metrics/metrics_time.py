...
class TimeMetrics(Metrics):
    ...
    def calculate_next_action_time(metrics):
        ...
        df_tmp = (
            df_tmp.groupby("kernel_id")
            .apply(lambda x: (x.time - x.time.shift(1)).shift(-1))
            .reset_index(level=0)
    ...
    def calculate_interruptions(metric_df):
        ...
        for i, row in metric_df.iterrows():
            if row.execution_start is not None:
                metric_df.loc[i, "interruptions"] = sum(
                    metric_df.time.between(
    ...
    def match_executions(self, kernel_df):
        ...
        for i, row in kernel_df.iterrows():
            ...
            if row.event == "finished_execute":
                ...
                if row.cell_index in [n[0] for n in execution_queue]:
                    ...
                    # TODO: normal fix?
                    latest_time = (
                        execution_queue[latest][2] if execution_queue[latest][2] > latest_time else latest_time
                    )
        ...
        return pd.DataFrame(
            all_executions,
            columns=["action_id", "cell_index", "execution_time", "execution_start", "matched_label", "result"],
        )
