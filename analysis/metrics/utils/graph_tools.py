...
def evolution_to_execution_path(
    ...
    deleted_cells = [
        x.log.get("cell_index") for x in evolution[min_snap_num:max_snap_num] if x.log.get("event") == "delete"
    ]
...
def evolution_to_graphviz(
    ...
    #
    edges, deleted_cells = evolution_to_execution_path(evolution, max_snap_num, min_snap_num, hash_string_num)

    graph.attr("node", weight="0")

    for cell_index in deleted_cells:
        graph.node(cell_index[:hash_string_num], fillcolor="lightgrey", style="filled")
...
def dataframe_to_graphviz(
    ...
    min_state_num: Optional[int] = None,
    max_state_num: Optional[int] = None,
    hash_string_num: int = 8,
    ...
    for i, (_, log_row) in enumerate(executions.iterrows()):
        ...
        params = {"label": f"{i}", "color": matplotlib.colors.to_hex(cmap(norm(i))) if colorful else "black"}
        cur_cell_index = cell_index[:hash_string_num]
...
def evolution_to_networkx(
    ...
    edges = evolution_to_execution_path(evolution, max_snap_num, min_snap_num, hash_string_num)
...
def draw_nx_graph(graph: nx.MultiDiGraph, fig_ax: [Tuple, None] = None) -> Tuple:
    if fig_ax is None:
    ...
    nx.draw_networkx_nodes(graph, pos, ax=ax, node_color="k", node_size=100, alpha=1, label=True)
