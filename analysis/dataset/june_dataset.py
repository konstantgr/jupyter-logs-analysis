...
class JuNEDataset:
    ...
    def to_evolution_dataframe(self, **kwargs) -> pd.DataFrame:
        ...
        kernel_dataframes = [
            self.get_kernel_states(kernel_id, **kwargs) for kernel_id in tqdm(self.df_june.kernel_id.unique())
        ]
    ...
    def get_notebook_state_by_id(self, action_id: int) -> pd.DataFrame:
        ...
        return df.groupby("kernel_id").get_group(kernel_id).groupby("state_num").get_group(state_num)

    @staticmethod
    def match_executions(cell_df):
        looking_for_finish = False
        found = {"executions": [], "unexecuted": [], "hagning_finish": []}
...
class NotebookState:
    ...
    def cells(self):
        return [
            ...
            for (idx, num) in self.index_order
            if idx in self.index_source_mapping.keys()
        ]
    ...
    def delete_cell(self, cell: Cell) -> None:
        ...
        current_num = (
            self.index_num_mapping[current_index] if current_index in self.index_num_mapping else cell.cell_num
        )
        if current_index in self.index_num_mapping:
    ...
    def execute_cell(self, cell: Cell) -> None:
        current_index, current_num = cell.cell_index, cell.cell_num
        if ((current_index, current_num) in self.index_order) and (current_index in self.index_num_mapping):
            return
...
def delete_duplicates(state: NotebookState) -> NotebookState:
    ...
    state.index_order = [p for i, p in enumerate(state.index_order) if i not in set(list_index_to_delete)]
    return state
